...
def begin_graphics( width=640, height=480, color=formatColor(0,0,0), title=None ):
    ...
    _root_window.bind( "<Button-1>", _click )
    _clear_keys()
...
def polygon( coords, color, filled = 1, smooth = 1, width = 1 ):
  c = []
  for coord in coords:
    c.append(coord[0])
    c.append(coord[1])
  ...
  if not filled:
    fcolor = ""
  return _canvas.create_polygon( c, outline=color, fill=fcolor, smooth = smooth, width=width )

...
class CrawlingRobotEnvironment(environment.Environment):
   ...
   def reset(self):
       ...
       self.crawlingRobot.positions = [self.crawlingRobot.getRobotPosition()[0]]
...
class CrawlingRobot:
    ...
    def draw(self):
        ...
        positionMsg = 'Position: %2.f' % self.getRobotPosition()[0]
        if 'vel_msg' in dir(self):
            self.canvas.delete(self.vel_msg)
            self.canvas.delete(self.pos_msg)
        self.vel_msg = self.canvas.create_text(700,200,text=velocityMsg)
        self.pos_msg = self.canvas.create_text(700,230,text=positionMsg)
